...
struct RewriteExternCallOpToDynamicImportCallOp
  ...
  LogicalResult matchAndRewrite(LLVM::CallOp callOp,
                                PatternRewriter &rewriter) const override {
    llvm::dbgs() << "\nRewriteExternCallOpToDynamicImportCallOp!\n";
...
void ConvertToLLVMPass::runOnOperation() {
  ...
  llvm::dbgs() << "\nConvertToLLVM: After Process Vectors!\n";
  module.dump();
  ...
  // Skip NPU-related ops for later processing
  target.addDynamicallyLegalOp<func::FuncOp>([](func::FuncOp op) {
    return op.getName().contains("npu_");
  });
  target.addDynamicallyLegalOp<func::CallOp>([](func::CallOp op) {
    return op.getCallee().contains("npu_");
  });

  llvm::dbgs() << "\nBefore applying LLVMPatterns!\n";
  module.dump();
  ...
  // Rewrite any NPU-related ops
  {
    RewritePatternSet npuPatterns(&getContext());
    populateNPUOpsPatterns(npuPatterns);
    if (failed(applyPatternsAndFoldGreedily(module, std::move(npuPatterns)))) {
      llvm::dbgs() << "\nNPUPatterns!\nFailed to applyPatternsAndFoldGreedilyPa\n";
      return signalPassFailure();
    }
    llvm::dbgs() << "\nAfter applying npuPatterns!\n";
    module.dump();
