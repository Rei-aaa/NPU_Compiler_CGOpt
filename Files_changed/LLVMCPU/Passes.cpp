...
static void addTileAndDistributePasses(OpPassManager &pm) {
  llvm::dbgs() << "addTileAndDistributePasses : START!\n";
  ...
  // CIMWise functions of adding CIMWiseKernelDispatch pass and CIMWiseTiledConv pass after fixing the parameters of tensors
  // This CIMWiseTiledConv pass is based on that unchanged TileAndDistributeToWorkgroupsPass
  // nestedModulePM.addNestedPass<func::FuncOp>(createCIMWiseKernelDispatchPass());  
  // nestedModulePM.addNestedPass<func::FuncOp>(createCIMWiseTiledConvPass());
  llvm::dbgs() << "addTileAndDistributePasses : END!\n";
}
...
static void addLowerToLLVMPasses(OpPassManager &passManager,
  ...
  // passManager.addPass(createConvertNPUOpsPass());
  // passManager.addPass(createConvertNPUCallPass());
...
void buildLLVMCPUCodegenConfigurationPassPipeline(OpPassManager &passManager) {
  {
    ...
    // CIMWise test//
    // Here has been tested for only changing the generated tile sizes from KernelDispatch pass
    modulePassManager.addNestedPass<func::FuncOp>(createCIMWiseKernelDispatchPass());
  }
  llvm::dbgs() << "buildLLVMCPUCodegenConfigurationPassPipeline start! \n";
